...
| `docs/purricane.md` | Kaleidoscope theme + desktop companions. Read before building the kaleidoscope, or the companion process and its packs (v0.7: Cap'n Capy first, kittens after, D145) |
| `docs/companion-art.md` | How a companion's poses become a sprite sheet. Cap'n Capy's frames are in `design/sprites/captain/` |
...
**v0.0 through v0.4b are built.** The window-engine spike returned **go** on the bond model (D45); its `bond.rs` was ported byte-identical, the spike repo has been archived, and `bond.rs` now evolves under its own tests like any other module (D66). v0.4b: chrome drawn from tokens in CSS (D72), Main owns playback and shows the spectrum on the radar ramp (D74), the 10-band EQ (D75), the playlist window, seams that glow and discharge (#9), 2x chrome (D76), the playlist's corner grip, the viz stream on its own pipe, the windowshade as a mini-player, Main as the one transport (D81), the download page and tagged Releases (D82), removal from the library (D83, D84), the captain's frames and the recipe that made them (`docs/companion-art.md`, `design/sprites/captain/`), minimise as a group (D86), the library to the tray (D87), and three bond-model rules that hand tests forced: a click is not a drag (D85), a title bar stays within reach (D88), a window never bonds over another's body (D89). The sprite renderer (#3) drew the shell chrome of all three windows from `skins/eyewall/` through `hp-skin/1` (D90, D92) and put the shade button on every title bar (#8); then Main's interior and the close button (D93, D94), the EQ's (D98), and the playlist's, with shuffle and repeat in its bar (D97, D99); and the glow became a person's to turn off (D100). That is the whole of v0.4b, tracked as its milestone on GitHub. **v0.5 is built** (2026-09-13): the `.wsz` loader, supported to a degree (D110); the headline, the skin maker, from a picture or painted by hand (D122, D129); your own EQ presets and `.eqf` import (D128); and Purricane, with its own skin and the kaleidoscope (D130, D132). **v0.6 is built** (2026-09-17), the week before the storm: the shipping ffmpeg is yt-dlp's own build, pinned, with the sidecars' licences in the zip (D133); the Cone theme, the radar loop behind the classic windows (D135); a chosen download folder (D136); the library roots watched while the app runs, letting go of a drive when it is ejected (D137); the storage budget, a meter, an optional ceiling and audio only as the way out (D138, D139); prep mode, Hurricane Party Planning, with downloads that wait out a lost connection (D140, D141); integrity checking, a fingerprint at import and a quiet pass after launch (D142); a flash drive that comes and goes, its tracks hidden while it is out and its root following it to a new letter (D143); and smart playlists, lists that fill themselves from a rule (D144). **v1.0 is next**, the control protocol frozen (`layout_changed`, the unbuilt commands, the docs and an example client), and then **v0.7**, the desktop companion with **Cap'n Capy** as its first pack; the kittens come after, as a second pack (D145). Both rows are in `decisions.md`, and both milestones on GitHub.
...
- **Leave the app running from the session, on the PR branch.** A background `pnpm tauri dev` on the branch means the owner tests the build the PR describes; hot reload carries frontend edits in and a Rust change rebuilds and relaunches. Never two dev servers: the second fails on the port but still starts a second exe on top of the first, sharing the database. `tools\dev.ps1` frees the port and a stray exe first, then starts one. `.claude/launch.json` hands that script and port 1420 to the desktop app's Browser pane, which otherwise guesses 5173, the Vite default
