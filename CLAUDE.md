...
- **Leave the app running from the session, on the PR branch.** A background `pnpm tauri dev` on the branch means the owner tests the build the PR describes; hot reload carries frontend edits in and a Rust change rebuilds and relaunches. Never two dev servers: the second fails on the port but still starts a second exe on top of the first, sharing the database. `tools\dev.ps1` frees the port and a stray exe first, then starts one
