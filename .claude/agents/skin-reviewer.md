...
5. **The 60 Hz path.** No CSS `filter` on the visualizer surface or any ancestor of it, whatever the manifest's `glow` says (D73), and no animated `box-shadow` on anything that moves per frame: the analyser, the seam, a drag. Chrome glow is the renderer's, from `accent`, and lives on chrome elements only; the analyser's own glow is pre-rendered in its ramp art. Animate `opacity` or `transform` on a composited layer. `prefers-reduced-motion` turns the seam discharge into an instant state change.
