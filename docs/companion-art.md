...
2. **Key the background out**: `tools\keyout.ps1 -In pose.png -Out idle-0.png -Size 1024`. It flood-fills in from the border, so a grey boombox on a green field survives. A model that can output a transparent PNG skips this step.
3. **Name the frames** `<state>-<n>.png` in one folder: `idle-0.png`, `idle-1.png`, `walk-0.png` … `walk-3.png`.
4. **Pack them**: `tools\sheet.ps1 -In <folder> -Out <packdir> -Frame 64 -Name "Cap'n Capy"`. It finds the tallest pose, scales every frame by that one factor (so a crouch does not grow to fill the cell), puts the feet on the bottom edge, and writes `sheet.png` plus a `companion.json` with every frame it placed. Nearest-neighbour by default, which keeps pixel art crisp; `-Smooth` for painted sources.
...
- **Scale drifts** between images even with a reference. `sheet.ps1` normalises on the tallest frame, so a slightly smaller walk frame is harmless; a pose that came out at half size will lose detail. Regenerate it rather than accept it.
...
The Eyewall skin's sprite sheet (#3, D73) is a different job and a generator is the wrong tool for it. Those sprites are **shapes** at one and two pixels of stroke: title bar, buttons in their four states, slider tracks and thumbs, drawn as alpha masks the renderer tints from the palette and glows from `--arc`. That is pixel-editor work (Aseprite, or anything with a grid and a pencil), from the rectangles `skin-manifest.md` fixes, and it goes under `skins/eyewall/`. The companion is the one that gets to be an illustration.
