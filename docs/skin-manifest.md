...
The `visualizer` **element** may say **`"shape": "round"`** (D132): the analyser is drawn inside the largest circle its box holds, and its well is a circle too. The default is `"rect"`. Purricane's badge is a 34 × 34 round visualizer, with its ring an `image` laid over it; an image takes no clicks, so the badge still cycles the display. The circle is a clip, never a mask or a filter, so D73 holds.
